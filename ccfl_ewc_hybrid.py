...
class CCFLMemoryEWC(nn.Module):
    def __init__(self, input_dim=784, hidden_dim=256, feat_dim=128,
                 n_contexts=10, context_dim=32, n_classes=10):
        super().__init__()
...
class EWCHook:
    def __init__(self, model, dataloader, lambda_ewc=5000):
        ...
        for data, target in dataloader:
            model.zero_grad()
            loss = F.cross_entropy(model.forward_classify(data), target)
            loss.backward()
...
# ============================================================
# Experiment 1: Split-MNIST with CCFL+EWC
# ============================================================

def experiment_split_mnist():
    log("=" * 76)
    log("  Experiment 1: Split-MNIST — CCFL+EWC vs All Baselines")
    log("=" * 76)
    ...
    # --- CCFL + EWC + Replay ---
    log("  [1/4] Training CCFL+EWC+Replay...")
    model = CCFLMemoryEWC(input_dim=784, hidden_dim=256, feat_dim=output_dim,
                           n_contexts=1, context_dim=32, n_classes=10)
    ...
    for t_id, (la, lb) in enumerate(tasks):
        ...
        for epoch in range(10):
            for data, target in task_loader:
                ...
                for hook in ewc_hooks:
                    loss = loss + hook.penalty(model)

                opt.zero_grad()
                loss.backward()
                opt.step()

        ewc_hooks.append(EWCHook(model, task_loader, lambda_ewc=1000))
    ...
    for t_id, (la, lb) in enumerate(tasks):
        ...
        for epoch in range(5):
            for data, target in task_loader:
                ...
                loss.backward()
                opt.step()
        ewc_list.append(EWCHook(ewc_model, task_loader, lambda_ewc=5000))
    ...
    log(f"\n  {'Task':>6s} | {'FT':>6s} | {'EWC':>6s} | {'ER':>6s} | {'Cls':>6s} | {'Mem':>6s}")
    log(f"  {'-'*6} | {'-'*6} | {'-'*6} | {'-'*6} | {'-'*6} | {'-'*6}")

    avg_ft, avg_ewc, avg_er, avg_cls, avg_mem = 0, 0, 0, 0, 0
    ...
    for t_id, (la, lb) in enumerate(tasks):
        ...
        correct_ft, correct_ewc, correct_er, correct_cls, total = 0, 0, 0, 0, 0
        with torch.no_grad():
            for data, target in task_loader:
                correct_ft += (ft_model(data).argmax(1) == target).sum().item()
                correct_ewc += (ewc_model(data).argmax(1) == target).sum().item()
                correct_er += (er_model(data).argmax(1) == target).sum().item()
                correct_cls += (model.forward_classify(data).argmax(1) == target).sum().item()
                total += len(target)
        ...
        avg_ft += acc_ft; avg_ewc += acc_ewc; avg_er += acc_er
        avg_cls += acc_cls; avg_mem += acc_mem

        log(f"  T{t_id:>4d} | {acc_ft:>6.3f} | {acc_ewc:>6.3f} | {acc_er:>6.3f} | {acc_cls:>6.3f} | {acc_mem:>6.3f}")

    n = n_tasks
    log(f"  {'Avg':>6s} | {avg_ft/n:>6.3f} | {avg_ewc/n:>6.3f} | {avg_er/n:>6.3f} | {avg_cls/n:>6.3f} | {avg_mem/n:>6.3f}")

    return {'FT': avg_ft/n, 'EWC': avg_ewc/n, 'ER': avg_er/n,
            'CCFL+EWC-Cls': avg_cls/n, 'CCFL+EWC-Mem': avg_mem/n}


# ============================================================
# Experiment 2: Permuted-MNIST with CCFL+EWC
# ============================================================

def experiment_permuted_mnist():
    log("\n" + "=" * 76)
    log("  Experiment 2: Permuted-MNIST — CCFL+EWC vs EWC")
    log("=" * 76)
    ...
    for t_id in range(n_tasks):
        ...
        avg_ewc += acc_ewc; avg_cls += acc_cls; avg_mem += acc_mem
        log(f"  T{t_id:>4d} | {acc_ewc:>8.4f} | {acc_cls:>8.4f} | {acc_mem:>8.4f}")

    n = n_tasks
    log(f"  {'Avg':>6s} | {avg_ewc/n:>8.4f} | {avg_cls/n:>8.4f} | {avg_mem/n:>8.4f}")

    return {'EWC': avg_ewc/n, 'CCFL+EWC-Cls': avg_cls/n, 'CCFL+EWC-Mem': avg_mem/n}


# ============================================================
# Experiment 3: Context-Dependent MNIST with CCFL+EWC
# ============================================================

def experiment_context_dependent():
    log("\n" + "=" * 76)
    log("  Experiment 3: Context-Dependent MNIST — CCFL+EWC Sequential")
    log("=" * 76)
    ...
    for ctx in range(n_contexts):
        ...
        for epoch in range(10):
            for data, target in torch.utils.data.DataLoader(train_dataset, batch_size=256, shuffle=True):
                ...
                mapped = torch.tensor([label_mappings[ctx][t.item()] for t in tm])
                cls_loss = F.cross_entropy(model.forward_classify(dm), mapped)
                ctx_ids = torch.full((len(dm),), ctx, dtype=torch.long)
                combined = ctx_ids * 10 + tm
                feat = model.forward_features(dm, ctx_ids)
                ctr_loss = supervised_contrastive_loss(feat, combined, 0.07)
                loss = cls_loss + 0.5 * ctr_loss
                if len(replay) > 0:
                    r_data, r_target, r_ctx = replay.sample(256)
                    r_mapped = torch.tensor([label_mappings[int(r_ctx[i].item())][int(r_target[i].item())]
                                              for i in range(len(r_target))])
                    r_cls_loss = F.cross_entropy(model.forward_classify(r_data), r_mapped)
                    r_combined = r_ctx * 10 + r_target
                    r_feat = model.forward_features(r_data, r_ctx)
                    r_ctr_loss = supervised_contrastive_loss(r_feat, r_combined, 0.07)
                    loss = loss + r_cls_loss + 0.5 * r_ctr_loss
        ...
        task_loader_for_ewc = torch.utils.data.DataLoader(
            [(d, label_mappings[ctx][t.item()]) for d, t in train_dataset if t.item() in base_digits],
            batch_size=256, shuffle=True)
        ewc_hooks.append(EWCHook(model, task_loader_for_ewc, lambda_ewc=500))
...
if __name__ == "__main__":
    log("=" * 76)
    log("  CCFL + EWC Hybrid Experiments")
    log("  Innovation: EWC protects backbone, CCFL provides context-conditional memory")
    log("=" * 76)

    r1 = experiment_split_mnist()
    r2 = experiment_permuted_mnist()
    r3 = experiment_context_dependent()
